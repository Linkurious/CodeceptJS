...
module.exports = function (config) {
  ...
  function getDir(test) {
    const sha256hash = crypto
      ...
      .digest('hex')
    const dir = path.join(reportDir, `record_${sha256hash}`)
    mkdirp.sync(dir)
    return dir
  }

  event.dispatcher.on(event.test.before, test => {
    const dir = getDir(test)
    mkdirp.sync(dir)
    stepNum = 0
    error = null
    slides = {}
  ...
  event.dispatcher.on(event.step.failed, async step => {
    await recorder.add('screenshot of failed test step', () => persistStep(step))
  })

  event.dispatcher.on(event.step.after, async step => {
    await recorder.add('screenshot of test step', () => persistStep(step))
  })

  event.dispatcher.on(event.test.failed, async (test, _err, hookName) => {
    await recorder.add('persist test', () => persist(test))
  })

  event.dispatcher.on(event.test.passed, test => {
    if (!config.deleteSuccessful) return persist(test)
    deleteDir(getDir(test))
  })
  ...
  async function persistStep(step) {
    ...
    if (step.name === 'slowRequest') return

    const dir = getDir(currentTest)
    const fileName = `${pad.substring(0, pad.length - stepNum.toString().length)}${stepNum}.png`
    const screenshotPath = path.join(dir, fileName)
    try {
      await helper.saveScreenshot(screenshotPath, config.fullPageScreenshots)
    } catch (err) {
      const message = `Error while saving screenshot for ${step}, file ${screenshotPath}: ${err.message}`
      console.warn(message, err)
      output.plugin(message)
      error = err
      return
    } finally {
      savedStep = step
  ...
  async function persist(test) {
    if (error) return

    const lastSavedStepIndex = test.steps.findIndex(step => step === savedStep)
    for (let i = lastSavedStepIndex; i < test.steps.length; i++) {
      const step = test.steps[i]
      console.log(`Force-persist step ${step} (stepNum=${stepNum})`)
